            ...
            @if($element_index || $contribution)
                <header class="ssnail-extra-info flex justify-between w-full absolute top-0 left-0 p-2">
                    <div class="ssnail-position">
                        @if($element_index)
                            <span class="ssnail-count inline-flex items-center justify-center w-8 h-8 scale-75 {{ ($highlight_user ?? false) ? 'bg-amber-400 text-white' : 'bg-amber-100 text-amber-600' }} font-bold rounded-full text-xs">{{ humanNumber($element_index) }}</span>
                        @endif
                    </div>
