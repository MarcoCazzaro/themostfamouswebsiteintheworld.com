                        ...
                        <tbody>
                            @foreach($found_users as $user)
                                <tr
                                    ...
                                    <td>
                                        <a href="{{ $user->url ?? '' }}">
                                            <img class="grow-0 shrink-0 w-8 h-8 object-cover rounded-full border border-amber-300 hover:border-amber-500 transition-all" src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}">
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="flex justify-end">
                                            <a href="{{ route('users.show', $user ?? null) }}" class="mr-3 text-amber-500"><i class="fas fa-eye"></i> {{ __('View') }}</a>
                                            @canImpersonate($guard = null)
                                                <a href="{{ route('impersonate', $user->id) }}" class="mr-3"><i class="fas fa-mask"></i> Impersonate</a>
                                            @endCanImpersonate
                                            @can('supadupaadminshit')
