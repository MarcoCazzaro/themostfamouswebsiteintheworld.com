        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <x-form method="POST" action="{{ $action }}">
                ...
                <div class="col-span-6 sm:col-span-3">
                    ...
                    <x-input id="name" name="name" type="text" class="mt-1 block w-full" autocomplete="name" value="{{ old('name') ?? $user->name ?? null }}"/>
                    <x-input-error for="name" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-3">
                    <x-label for="email" value="{{ __('Email') }}" />
                    <x-input id="email" name="email" type="email" class="mt-1 block w-full" autocomplete="email" value="{{ old('email') ?? $user->email ?? null }}"/>
                    <x-input-error for="email" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-3">
                    <x-label for="slug" value="{{ __('Slug') }}" />
                    <x-input id="slug" name="slug" type="text" class="mt-1 block w-full" autocomplete="slug" value="{{ old('slug') ?? $user->slug ?? null }}" />
                    <x-input-error for="slug" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-3">
                    <x-label for="slug" value="{{ __('Type') }}" />
