        ...
        <section class="mb-16">
            ...
            @foreach($most_famous_tags->take(13) as $tag)
                ...
                <div class="ssnail-most-famous-people-by-tag mb-16" data-tag-id="{{ $tag->id }}">
                    <h3 class="font-semibold bg-white rounded-lg border shadow-md">Most famous {{ $subject }} for <span class="text-amber-500">{{ $tag->name }}</span> <a class="float-right text-gray-500" href="{{ route('users.most-famous-' . $subject . '.show', $tag) }}">See full ranking <i class="fas fa-arrow-right"></i></a></h3>
                    @if(!empty($users))
                        @livewire('users-list', ['currentUsers' => $users, 'highlightFirst' => true, 'showPosition' => true])
                    @else
